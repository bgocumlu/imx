...
function cmakeHotreloadCombined(projectName: string, hasPersistence: boolean): string {
...
function buildMainCpp(projectName: string, extraIncludes: string, callbacks: string, hasPersistence: boolean): string {
...
function buildMainCppHotreload(projectName: string, extraIncludes: string, callbacks: string, hasPersistence: boolean): string {
    const includeBlock = extraIncludes ? '\n' + extraIncludes : '';
    const persistenceInclude = hasPersistence ? '\n#include "persistence.h"' : '';
...
function buildAppStateH(headers: string[], fields: string, nlohmannMacroFields: string[] | null): string {
...
function buildAppTsx(windows: string): string {
...
function deduplicateFields(fieldsBlock: string): string {
...
function deduplicateTsFields(fieldsBlock: string): string {
    const seen = new Set<string>();
    const result: string[] = [];
    for (const line of fieldsBlock.split('\n')) {
        const trimmed = line.trim();
...
export function generateCombined(featureNames: string[], projectDir: string, projectName: string): void {
    // 1. Resolve feature objects
    const featureMap = new Map<string, FeatureModule>();
    for (const f of FEATURES) featureMap.set(f.name, f);
    ...
    const allCppFields: string[] = [];
    const allCppHeaders: string[] = [];
    const allTsFields: string[] = [];
    const allCallbacks: string[] = [];
    const allWindows: string[] = [];
    const allExtraFiles: Record<string, string> = {};
    let nlohmannFields: string[] | null = null;

    for (const feat of ordered) {
        for (const inc of feat.includes) {
            if (!allIncludes.includes(inc)) allIncludes.push(inc);
        }
        if (feat.appStateCppFields) allCppFields.push(feat.appStateCppFields);
        for (const h of feat.appStateCppHeaders) {
            if (!allCppHeaders.includes(h)) allCppHeaders.push(h);
        }
        if (feat.appStateTsFields) allTsFields.push(feat.appStateTsFields);
        if (feat.callbacks) allCallbacks.push(feat.callbacks);
        if (feat.tsxWindow) allWindows.push(feat.tsxWindow);
