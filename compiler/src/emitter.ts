...
function findTopLevelTernary(expr: string): { question: number; colon: number } | undefined {
    let depthParen = 0;
    let depthBracket = 0;
    let depthBrace = 0;
    let inString = false;
...
function isCharPtrExpression(expr: string): boolean {
...
function asCharPtr(expr: string): string {
    const trimmed = expr.trim();
    if (isCharPtrExpression(trimmed)) return trimmed;
...
function assignStyleExpr(varName: string, styleExpr: string, indent: string, lines: string[]): void {
...
function buildWidgetStyleVar(styleExpr: string | undefined, widthExpr: string | undefined, indent: string, lines: string[]): string | null {
    if (!styleExpr && !widthExpr) return null;
    const varName = `style_${styleCounter++}`;
    lines.push(`${indent}imx::Style ${varName};`);
    if (styleExpr) {
        assignStyleExpr(varName, styleExpr, indent, lines);
...
function nextWidgetTemp(prefix: string): string {
    return `_${prefix}_${widgetTempCounter++}`;
}

function emitActionStatements(statements: string[] | undefined, lines: string[], indent: string): void {
...
function emitItemInteractionBefore(item: IRItemInteraction | undefined, lines: string[], indent: string): void {
    if (!item?.autoFocus) return;
    lines.push(`${indent}if (${item.autoFocus}) imx::renderer::request_keyboard_focus();`);
}

function emitItemInteractionAfter(item: IRItemInteraction | undefined, lines: string[], indent: string): void {
    if (!item) return;
    ...
    const callbackChecks: Array<[string[] | undefined, string]> = [
...
function emitBoolWidgetCall(
    callExpr: string,
    item: IRItemInteraction | undefined,
    lines: string[],
    indent: string,
    resultVar?: string
): string | null {
...
function emitBeginContainer(node: IRBeginContainer, lines: string[], indent: string): void {
    ...
    switch (node.tag) {
        ...
        case 'MultiSelect': {
            ...
            if (node.props['boxSelect'] === 'true') flagParts.push('ImGuiMultiSelectFlags_BoxSelect1d');
            if (node.props['boxSelect2d'] === 'true') flagParts.push('ImGuiMultiSelectFlags_BoxSelect2d');
            if (node.props['boxSelectNoScroll'] === 'true') flagParts.push('ImGuiMultiSelectFlags_BoxSelectNoScroll');
            if (node.props['clearOnClickVoid'] === 'true') flagParts.push('ImGuiMultiSelectFlags_ClearOnClickVoid');
            const flags = flagParts.length > 0 ? flagParts.join(' | ') : '0';
...
function emitSpacing(node: IRSpacing, lines: string[], indent: string): void {
    emitLocComment(node.loc, 'Spacing', lines, indent);
    lines.push(`${indent}imx::renderer::spacing();`);
}

function emitDummy(node: IRDummy, lines: string[], indent: string): void {
...
function emitSameLine(node: IRSameLine, lines: string[], indent: string): void {
...
function emitNewLine(node: IRNewLine, lines: string[], indent: string): void {
...
function emitShortcut(node: IRShortcut, lines: string[], indent: string): void {
