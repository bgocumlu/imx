...
function isStringLikeExpr(expr: ts.Expression, ctx: LoweringContext): boolean {
...
function coerceExprToCppString(expr: ts.Expression, ctx: LoweringContext): string {
...
function normalizeIdScopeAttrs(
    ...
    if (!scopeExpr) return normalized;
...
function lowerItemInteraction(
    attrs: Record<string, string>,
    rawAttrs: Map<string, ts.Expression | null>,
    ctx: LoweringContext
): IRItemInteraction | undefined {
    const item: IRItemInteraction = {};

    if (attrs['autoFocus']) item.autoFocus = attrs['autoFocus'];
    if (attrs['tooltip']) item.tooltip = attrs['tooltip'];
    if (attrs['scrollToHere']) item.scrollToHere = attrs['scrollToHere'];
    if (attrs['cursor']) item.cursor = attrs['cursor'];

    const callbackAttrs: Array<[
        'onHover' | 'onActive' | 'onFocused' | 'onClicked' | 'onDoubleClicked',
        string
    ]> = [
        ['onHover', 'onHover'],
        ['onActive', 'onActive'],
    ...
    if (isHostComponent(name)) {
        ...
        if (def.isContainer) {
            ...
            if (containerTag === 'DockSpace' || containerTag === 'Window') {
                const hasMenuBar = node.children.some(c =>
                    ts.isJsxElement(c) && ts.isIdentifier(c.openingElement.tagName) && c.openingElement.tagName.text === 'MenuBar'
                );
                if (hasMenuBar) attrs['hasMenuBar'] = 'true';
            }
...
function lowerShortcut(attrs: Record<string, string>, rawAttrs: Map<string, ts.Expression | null>, body: IRNode[], ctx: LoweringContext, loc: SourceLoc): void {
    ...
    body.push({ kind: 'shortcut', keys, action, loc } as IRShortcut);
}
...
function resolvePropertyAccessType(expr: ts.PropertyAccessExpression, ctx: LoweringContext): IRType | null {
    if (!ctx.propsParam) return null;
    ...
    if (!currentType || currentType === 'callback') return null;

    for (let i = 1; i < parts.length; i++) {
        if (!ctx.externalInterfaces) return null;
        const iface = ctx.externalInterfaces.get(currentType);
        if (!iface) return null;
        const nextType = iface.get(parts[i]);
        if (!nextType || nextType === 'callback') return null;
...
function lowerVectorInput(
    ...
    const item = lowerItemInteraction(attrs, rawAttrs, ctx);
    const base: any = { kind: family, label, count, stateVar, valueExpr, directBind, onChangeExpr, width, style, item, loc };
