...
### 1. Dynamic `disabled` expressions on `Button` (fixed)
...
### 2. `<ID scope={stringExpr}>` now emits string `PushID` correctly (fixed)
...
### 3. String concatenation inside `<Text>{...}</Text>` (fixed)
...
### 4. Numeric text interpolation in child components (fixed)
...
### 5. Individual scalar props preserve C++ type info (fixed)
...
### 6. MultiSelect bound prop detection (fixed)
...
### 7. Left-click context menus on interactive items (fixed)
...
### 8. Non-root component imports (fixed)
