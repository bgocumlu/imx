...
### 1. String concatenation inside `<Text>{...}</Text>` (fixed)
...
### 2. Numeric text interpolation in child components (fixed)
...
### 3. Individual scalar props preserve C++ type info (fixed)
...
### 4. MultiSelect bound prop detection (fixed)
...
### 5. Left-click context menus on interactive items (fixed)
...
### 6. Non-root component imports (fixed)
